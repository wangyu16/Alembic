...
6. **Copy = adapt.** When a block is copied into another package, the copy MUST receive a new ID plus an `adaptedFrom` reference `{ packageId, blockId, snapshot? }` to the source block. This is the basis for block-level attribution and upstream/downstream improvement loops. Snapshots shipped in M15 (snapshots-as-tags, §10), and the `snapshot` field is plumbed through `adaptBlocksInto` ([`packages/package-ops/src/adaptation.ts`](../../packages/package-ops/src/adaptation.ts)); pinning a block-level adaptation to a specific source snapshot is the remaining thin follow-up (M15.5).
...
| `chapters` | array of `{ ... }` | no (additive) | Ordered chapter index. **Absent = a single implicit chapter** (the default study-guide file); old packages stay valid with no migration. See [course-structure.md](course-structure.md). |
| `accessibility` | object | no (additive) | Last accessibility audit result (`status`, `errorCount`, `warningCount`, `checkedAt`). **Absent = unknown** (never checked). |
| `adaptedFrom` | object | no (additive) | Package-level adaptation lineage, set when this package was forked/adapted from another (`packageId`, `title?`, `snapshot?`, `license`, `attribution`, `url?`). **Absent = an original work.** The full record lives in [`adaptation.ts`](../../packages/package-contract/src/adaptation.ts) (`AdaptationSourceSchema`). |
...
> **Spec note:** adaptation provenance is now implemented (Phase 5). The contract layer defines `AdaptationSourceSchema` and the `canAdapt` CC-license matrix in [`adaptation.ts`](../../packages/package-contract/src/adaptation.ts); the ops layer writes per-block lineage as `AdaptedBlockRef` records into `provenance/adaptations.json` (`adaptBlocksInto` in [`packages/package-ops/src/adaptation.ts`](../../packages/package-ops/src/adaptation.ts)). Still pending in `package-contract`: source records and embedded-source hashes at minimum (M4 records the hash on export).

## 10. Out of scope for v1 / reserved

Named here so future versions slot in without disturbing v1 packages:

- **Snapshots-as-tags.** A snapshot is a named, immutable package version implemented as a Git tag, with restore/compare/cite semantics and optional DOI minting ([goal.md §5](../goal.md)). Shipped in M15 (`createSnapshotAction` over `createTag`/`listTags`, `SnapshotsPanel`); the contract hooks — `adaptedFrom.snapshot`, the `CITATION.cff` allowlist entry — are in use, with `AdaptationSource.snapshot` plumbed through `adaptBlocksInto`. Remaining: block-level snapshot-pinning (M15.5) and DOI minting. (v0.1 also ships *restore to a previous save*.)
