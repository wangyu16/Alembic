...
> ⚠️ **User governance (UG) — deploy order matters, in BOTH directions**
> ([user-governance.md](specs/user-governance.md) §4a). *(Subtasks are numbered
> `UG0…UG6`, not `G0…`, because `G1–G8` already names the durable editor
> guardrails below. The commit `24c1da6` predates the rename and says `G0`.)*
> **UG0 is code-before-migration** — the install callback must already use the
> service client when `0016` revokes the grant. **UG1–UG3 is
> migration-before-code** — the AI gate calls `can_use_ai()` and fails *closed*,
> so shipping it before `0017` is applied stops AI for everyone, owner included.
> Sequence: deploy UG0 → apply `0016` → apply `0017`+`0018` → deploy UG1–UG3.
>
> Disabling a user is Supabase's native ban (`auth.users.banned_until`), **not** a
> `profiles.status` column — one source of truth, nothing to drift. A JWT cannot
> be revoked, so a ban leaves a ≤1h residual window; `is_active_user()` inside
> the write RLS policies closes it with zero application call sites, which is
...
been copied into **four** modules now have **one owner**; `adaptGivenBlocksInto`, `applyUpstreamUpdate` and
`applyProposedChangeSet` gained single-`putFiles` atomicity for free; and the last silent-divergence path —
`saveStudyGuideAction`, still store-first and still called by the shell — was migrated (its three
"saved here but not synced" warnings are gone; an unreachable published package now refuses). **T21:** a
newly created package is now **two files** (`alembic.json`, `LICENSE`) with an explicit first chapter in the
manifest and **no seeded prose**; `createChapter` writes no file at all. **Third slot-drift bug fixed:**
`deleteChapter` orphaned four of five chapter documents — the delete set is now slot-derived like the
rename. **T22/T21b:** Replace on a chapter document is now **create-or-replace**, so replacing a
never-opened document works (C4), any filename normalizes to the slot's canonical path with an
educator-facing note ("Saved as the slides for …", C5), and the UI vocabulary is bridged to the contract's
slots by a `Record` so a drift is a compile error. **T14b** (integrator): chapter ops return the updated
manifest; the post-op re-reads are gone. **Behavior change recorded:** a freshly created package no longer
passes the release gates — correct under "empty slots never publish", and the release-gate tests were
reframed rather than patched. Integrator also repaired 14 test fixtures that assumed the old seed (they now
author content through the real save path). Verified: typecheck + **1022 tests** + web build green.
...
surfaces + a new TTL staging bucket), the **repo-first write-through rule** (validate → commit → project →
record SHA; fail-loudly, no silent local-only writes; trial = DB-only branch), slots-not-placeholders, the
job-scoped raw-materials policy, and the **R2 verdict: skipped** with three recorded revisit triggers.
Implementation plan: [StorageWritePathPlan.md](StorageWritePathPlan.md) (Phases H hotfixes → W
write-through → S slots → P populate/staging → V verification). Acceptance:
...
[reports/workspace-issues-2026-08-28.md](reports/workspace-issues-2026-08-28.md). Headline: **the manifest
exists as two writable copies** (sandbox `alembic.json` file vs `packages.manifest` column) with split
writers — chapter ops write only the file; theme/course-info/a11y/term actions rebuild from the stale
column and clobber both + GitHub → chapters vanish (F1, critical). Also: Vercel's ~4.5 MB function body
cap 413s real zips before `/api/populate-package` runs, failures poison the pristine gate so retries are
...
source**/Reconcile) with Git translations, a permanent exclusion list (branch/merge/PR/commit-message/
rebase — each with an educator-facing replacement), the **two-pointer model** (working head + publish
pointer; no branches ever), the **no-merge divergence policy** (keep/take/AI-merge → forward commit), and
**docId-keyed history** (elevates the R2 registry to a version-control requirement). Consequence: the
**lean-`.md` committed source is permanent** — Roadmap R1/E3's ".md.html as committed source" target is
dropped (E3's in-file editing UX proceeds on generated surfaces). Verified against the product vision
(coverage table in the contract §8); rationale in [DecisionLog.md](DecisionLog.md) (2026-08-28). Docs-only
change (spec + DecisionLog + Roadmap amendments + this entry).
...
**Licensing tiers + copyright attestation (2026-07-12):** the platform is free/open; a package's *license*
and its *copyright-cleanliness* are now treated as separate axes. A new **`ALL-RIGHTS-RESERVED`** license
value (`manifest.ts` `LicenseSchema` + `isOpenLicense()` predicate) means *unlicensed* — a package can be
created, published to its own site, and used for a class, but **cannot be listed on Discover**. Enforcement:
`registerPackageAction` ("List publicly") now hard-requires an **open license** + an **educator
copyright attestation** (recorded in the event log); the publish confirm and the List-publicly confirm both
carry an **attest + warn** copyright prompt (own-site publishing stays allowed — the educator's
responsibility). ARR gets a plain "all rights reserved" `LICENSE` body + rights notice (no CC deed); JSON-LD
