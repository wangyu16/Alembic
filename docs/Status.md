...
> **In flight — Phase 9 editor overhaul, Phase 0 (foundation fixes).** A pilot UI/UX pass (homepage, package lifecycle, publish flow, chapter management, per-chapter history) led into a full design for rebuilding the editor as the replaceable client: [workspace-editor-overhaul.md](specs/workspace-editor-overhaul.md) (decisions + six-seam conflict audit + guardrails G1–G8) and [Roadmap.md](Roadmap.md) Phase 9. **Now implementing the audit's must-fixes on the current editor** (they're latent bugs the overhaul would amplify): **G1** activate `assertPublicReference` (two-repo reference leakage), **G5** chapter-aware `.md.html` export, then G2 re-import ID reconciliation. Track progress in the spec's §9/§10. This is *how we got here*: the framework (Phases 0–7) is done and tested; the editor surface is being hardened then overhauled.
