...
| # | Sub-module | Verify by | Status |
| --- | --- | --- | --- |
| 11.1 | Ketcher editor integration (draw/edit) → `.ketcher.svg` carrier | draw a structure; SVG + embedded source saved | 🔄 self-hosted standalone build (`pnpm fetch:ketcher` → `apps/web/public/ketcher/`, gitignored, 96 MB, v3.12.0) embedded in a lazy same-origin iframe (`ketcher-editor.tsx`); `saveStructureAssetAction` → `writeAsset`. **Build delivery verified live** (Vercel build command runs the fetch; `/ketcher/standalone/index.html` → 200 on alembic.orz.how). Remaining: interactive draw→save→insert→preview round-trip pass |
...
| 11.3 | Structure rendering in preview / site / exports (as `<img>`, inert) | structure renders in preview and the published site | 🔄 preview rewrites `materials/…` refs → `GET /api/asset/[pkg]/[...path]` (store-served, public-only). Published-site path resolution + permalink pinning with M15 |
| 11.4 | AI alt-text from structure source (chemistry-first a11y) | alt text generated then reviewed (Tier-2) | ✅ ai-assist `suggestStructureAltText` (27 tests) + `suggestStructureAltTextAction`; "Describe with AI" prefills an editable alt field (educator reviews before save) |

*Exit:* a chemistry section with a drawn, reusable structure publishes with alt text. **Code complete; the Ketcher canvas + end-to-end insert/preview need an interactive browser pass (and the build vendored on the deploy target).**
