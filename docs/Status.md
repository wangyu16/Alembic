...
> **In flight — Phase 9 editor overhaul, Phase 0 (foundation fixes).** A pilot UI/UX pass (homepage, package lifecycle, publish flow, chapter management, per-chapter history) led into a full design for rebuilding the editor as the replaceable client: [workspace-editor-overhaul.md](specs/workspace-editor-overhaul.md) (decisions + six-seam conflict audit + guardrails G1–G8) and [Roadmap.md](Roadmap.md) Phase 9. **Implementing the audit's must-fixes on the current editor** (they're latent bugs the overhaul would amplify): **G1 ✅ landed** — `assertPublicMarkdownReferences` enforced in `saveStudyGuide` (the chokepoint for human/AI/coherence edits) + a publish "References" release gate + adversarial tests (two-repo reference leakage closed); **G5 (export) ✅ landed** — the `.md.html` export route is chapter-aware (`?chapter=<slug>`, editor passes the active chapter; no more hardcoded default file). **G2 ✅ landed** — `.md.html` re-import reconciles by block ID (`reconcileImportedBlocks`): re-uploaded sections replace in place with IDs preserved, new sections append, absent ones kept — no more append-with-null duplication. **Guardrails: G1/G2/G3/G5/G6-durable/G8/G4 ✅ landed; **G7 ✅** — `@alembic/editor-kit` package: the unified `EditorModule`/`EditorContext`/`EditorHandle` interface + `EditorRegistry` (framework-agnostic; carriers §3 updated). **Phase 1 web actions built** (verify live once Portkey is on Vercel): `generate/saveCourseDescriptionAction` (G6 end-to-end via `setCourseDescription` → manifest/LRMI/portal derive); `proofreadChapterAction` (spelling/grammar → Tier-2 `editor-ai-edit` in the review queue, applied by the G3 branch). **Phase 3 shell scaffold landed** — `/workspace/[packageId]/edit` (opt-in "New editor (beta)" link): three panes (chapters | 7-category rail | editor), Course-home description (AI generate/save), per-section content editor (validated save); other categories route to the classic editor for now. Classic editor stays default until parity. **Editor overhaul implemented (Phases 0–4), statically verified.** The new three-pane shell at `/workspace/[packageId]/edit` is now the **default** "Open editor" (classic reachable via its "Classic editor" link). All 7 categories: concept-map/assessment-guide/private as validated markdown; Course content per-section + assembled preview; Assets create/edit via the **extracted ketcher/plot editor modules** (`@alembic/editor-kit` + `lib/editor-modules`); Slides + Practice generate-then-own; Course description + AI. **In-editor AI** (propose → before/after diff → approve → validated save). Chapter management in the rail. Per-format Agent Skills shipped in `skills/`. **Operator verification (deployed):** live AI flows need Portkey on Vercel; Ketcher/Plotly canvases need the vendored builds + a browser. Bulk `.docx`/`.pptx` import stays the reserved worker-tier seam. **Shell refinements:** full-width layout with both left panes (chapters + category rail) collapsible — the chapter list starts collapsed; content sections in the chapter editor are individually collapsible and start collapsed (Expand-all toggle). **Render theme = light-academic-1 ("Light Academic I") for the light theme**, dark-elegant for dark; the selected theme is now threaded consistently through preview, `.md.html`/artifact export, **and the published GitHub-Pages site** (`buildSite`/`buildCourseSite` previously hardcoded dark) — vendor swap via `scripts/vendor-light-theme.mjs`, locked with a `course-site` theme-consistency test. **Local Studio (`/studio`) restyled to match the new editor** — full-height panel layout, icon-forward header action cluster, source + live-preview panes. **Unsaved-changes guard** (`lib/use-unsaved-guard.ts`): while any editor is dirty, the browser's native `beforeunload` prompt arms (covers tab close / reload / external nav) and in-app link clicks are intercepted in the capture phase (covers switching chapter/section/page); download/`/export`/`/api/`/new-tab/in-page links are exempt. Wired into the new shell's ContentEditor/FileEditor/CourseHome/AssetEditor and the Studio (which also confirms before New note / Open file). Verified in-browser (dirty chip + beforeunload + link-intercept + exemptions). **Publishing moved into the new editor's header** — the classic `PublishHeader` (① Save to GitHub, ② Publish/Update web page + copy-link, List publicly) is now reused verbatim at the top-right of the new shell; the new server page computes the same publishing state (connected/published/gh-pages siteUrl/registration/installUrl) and lifts each pane's unsaved flag up so "Save to GitHub" is blocked until the active edit is saved. The GitHub-App install callback now resumes on the new editor (`/workspace/{id}/edit?publish=1`) since it's the default "Open editor". Panes are now keyed by file so switching chapter/category mounts a fresh editor instance (no stale state). **Phase 1 AI durable functions ✅** (provider-injected, stub-tested; gateway wiring is operator-side): `generateCourseDescription` (feeds `setCourseDescription`/G6), `proofread` (spelling/grammar), coherence **lenses** `symbols` + `narrative-drift` (additive to `COHERENCE_FINDING_KINDS` + the agent prompt), routed fast-tier. G4 = `forkPackage` (whole-package fork: re-minted block IDs, remapped references, `adaptedFrom` lineage, fresh private partition; +4 tests) + `forkOwnPackageAction` (same-owner). G8 = `ai`-entitlement enforced at `governedProvider` + CI provider-seam guard; G3 = `editor-ai-edit` kind + `ai.edit.requested` event + carrier-agnostic `applyEditorEdit` + generic apply branch. **Side task added:** one Agent Skill per dual-extension format (cross-platform round-trip; [carriers-and-assets.md §7](specs/carriers-and-assets.md)), built alongside the Phase-2 editor-module extraction. **Phase 1 underway** — **G6 metadata single-source durable core ✅**: `package-ops/metadata.ts` (`setCourseDescription` writes the canonical `metadata/course.md` and derives `manifest.description` → LRMI/portal; `deriveDescription` pure, +8 tests). Next Phase-1 steps (need the AI gateway / Course-pane UI): AI generation of the description + a `stale-artifact` drift check; then coherence lenses, spelling/grammar, permalinks. Track progress in the spec's §9/§10. This is *how we got here*: the framework (Phases 0–7) is done and tested; the editor surface is being hardened then overhauled.
