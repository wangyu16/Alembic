...
| # | Sub-module | Verify by | Status |
| --- | --- | --- | --- |
| 11.1 | Ketcher editor integration (draw/edit) → `.ketcher.svg` carrier | draw a structure; SVG + embedded source saved | 🔄 self-hosted standalone build (`pnpm fetch:ketcher` → `apps/web/public/ketcher/`, gitignored, 96 MB, v3.12.0) embedded in a lazy same-origin iframe (`ketcher-editor.tsx`); `saveStructureAssetAction` → `writeAsset`. **Canvas needs live browser verification** (can't run in CI) |
