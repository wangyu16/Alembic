...
export async function publishSiteAction(
  ...
  try {
    ...
    // Module S: per-chapter self-contained offline download (owner decision).
    // Generated by the worker (falls back to a rendered in-process build when no
    // worker is configured — still a valid offline copy). Best-effort per
    // chapter: a generation hiccup skips that download, never blocks publishing.
    const downloadFiles: SiteFile[] = [];
    for (const ch of chapters) {
      try {
        ...
        const path = `downloads/${ch.slug}.md.html`;
        ch.downloadHref = path;
        downloadFiles.push({ path, content: html });
      } catch {
        /* skip this chapter's offline download; the reading page still ships */
      }
    }
